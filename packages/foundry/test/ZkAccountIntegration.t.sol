...
contract ZkAccountIntegrationTest is Test {
    ...
    function _setupUserData(address user, string memory proofStr, string memory rootStr, string memory saltStr) internal {
    ...
    function testEndToEndAccountCreationAndUsage() public {
        ...
        vm.prank(alice);
        ZkAccount aliceAccount = factory.createAccount(
            userData[alice].proof,
            userData[alice].root,
            userData[alice].salt
        );
    ...
    function testMultipleUsersAccountCreation() public {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            accounts[i] = factory.createAccount(
                userData[users[i]].proof,
                userData[users[i]].root,
                userData[users[i]].salt
            );
    ...
    function testAccountInteractions() public {
        ...
        vm.prank(bob);
        ZkAccount bobAccount = factory.createAccount(
            userData[bob].proof,
            userData[bob].root,
            userData[bob].salt
        );
    ...
    function testAccountFactoryWithAutoSalt() public {
        ...
        ZkAccount account1 = factory.createAccountWithAutoSalt(
            userData[alice].proof,
            userData[alice].root
        );
        ...
        ZkAccount account2 = factory.createAccountWithAutoSalt(
            userData[alice].proof,
            userData[alice].root
        );
    ...
    function testSecurityAgainstInvalidProofs() public {
        ...
        factory.createAccount(
            userData[attacker].proof,
            userData[attacker].root,
    ...
    function testAccountPersistenceAndRedeployment() public {
        ...
        vm.prank(alice);
        ZkAccount account1 = factory.createAccount(
            userData[alice].proof,
            userData[alice].root,
            userData[alice].salt
        );
        ...
        vm.prank(alice);
        ZkAccount account2 = factory.createAccount(
            userData[alice].proof,
            userData[alice].root,
            userData[alice].salt
        );
    ...
    function testFuzzIntegration(bytes memory fuzzProof, bytes32 fuzzRoot, bytes32 fuzzSalt) public {
        ...
        vm.assume(keccak256(abi.encodePacked(fuzzProof, fuzzRoot)) != keccak256(abi.encodePacked(userData[alice].proof, userData[alice].root)));
        vm.assume(keccak256(abi.encodePacked(fuzzProof, fuzzRoot)) != keccak256(abi.encodePacked(userData[bob].proof, userData[bob].root)));
